...
/**
 * ****************************************************************
 * 文件名称:jiuyiAutoFitTextView.java
 * 作    者:Created by zhengss
 * 创建时间:2018/5/16 13:45
 * 文件描述:实现文本内容自动调整字体大小以适应TextView的大小
 * 注意事项:必须加 Android:lines=”1” 属性 不能用 android:singleLine=”true” ，切记!!
 * ****************************************************************
 */

public class JiuyiAutoFitTextView extends android.support.v7.widget.AppCompatTextView {
    //unit px
    private static float DEFAULT_MIN_TEXT_SIZE = Res.Dip2Pix(6);
    private static float DEFAULT_MAX_TEXT_SIZE = Res.Dip2Pix(18);
    // Attributes
    private TextPaint testPaint;
    private float minTextSize;
    private float maxTextSize;

    public JiuyiAutoFitTextView(Context context) {
        super(context);
        initialise();
    }

    public JiuyiAutoFitTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initialise();
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        JiuyiLog.e("TagSizeChange","new("+w+","+h+") old("+oldw+""+oldh+")");
        if (w != oldw || h != oldh) {
