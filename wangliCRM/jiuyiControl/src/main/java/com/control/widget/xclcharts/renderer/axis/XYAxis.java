 ...
 * @version 1.0
 */
package com.control.widget.xclcharts.renderer.axis;
	...
	protected void renderHorizontalTick(float chatLeft,float plotLeft,
		...
		if (isShowAxisLabels()) {			
			renderHorizontalTickLabels(chatLeft,plotLeft,canvas,
					 	 labelStartX+Res.Dip2Pix(10),labelStartY,markeStopX,text );
		}
				
	}
	
	//画标签
	protected void renderHorizontalTickLabels(float chatLeft,float plotLeft,Canvas canvas,
		...
		case BOTTOM:
			...
			if(this.isShowAxisLabels())
			{
				...
				labelsStartY = marksStopY + getTickLabelMargin()
						+ DrawHelper.getInstance().getPaintFontHeight(paint)
						/ 3;
			}
			break;
		default:
...
//					getTickLabelPaint());
			...
			Paint paint=getTickLabelPaint();
			paint.setTextSize(35);
			currY=currY+ Res.Dip2Pix(5);
			DrawHelper.getInstance().drawText(canvas,paint,getFormatterLabel(text),labelX,currY);
			//定制化显示格式			
		/*	DrawHelper.getInstance().drawRotateText(getFormatterLabel(text),
					labelX, currY, //labelsStartY,
					getTickLabelRotateAngle(), canvas,
