package com.control.widget.smallchart.compute;
...
public class ComputeXAxis<T extends IBarLineCurveData> extends Compute {
    ...
    public void computeXAxis(ArrayList<T> mBarLineCurveDatas){
        for (int i=0; i<mBarLineCurveDatas.size();i++){
            ...
            float minX =0;
            if (length>0){
                minX=maxX=mBarLineCurveData.getValue().get(0).x;
                for(int j=1;j<length;j++){
                    if(mBarLineCurveData.getValue().get(j).x>maxX){
                        maxX=mBarLineCurveData.getValue().get(j).x;
                    }
                    if(mBarLineCurveData.getValue().get(j).x<minX){
                        minX=mBarLineCurveData.getValue().get(j).x;
                    }
                }
