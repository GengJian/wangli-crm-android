...
import com.control.widget.datepicker.wheelview.adapter.WheelViewAdapter;
...
public class WheelView extends View {
	...
	// zhengss 20170120 循环的可选择的最大值
	int maxvalue = 4;
	// zhengss 20170120 是否正在按下，如果手指弹框，则为false
	...
	public void setBackgroundResources(int wheelBackground,int wheelForeground,int centerRectColor){
		this.wheelBackground = wheelBackground;
		this.wheelForeground = wheelForeground;
	...
	WheelScroller.ScrollingListener scrollingListener = new WheelScroller.ScrollingListener() {
		...
		public void onScroll(int distance) {
			doScroll(distance);
			//zhengss 20161124 height是超过多少就停止滚动,防止滚动的距离太大显得突兀
...
//		paint.setColor(getResources().getColor(R.color.province_line_border));
		paint.setColor(centerRectColor);
		// 设置线宽
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_CANCEL:
				isTouching = false;
				setCurrentItem(Math.min(maxvalue, currentItem), false);
				if (!isScrollingPerformed) {
	...
	private void doScroll(int delta) {
		...
		if (pos != currentItem) {
			//zhengss 20170120 定位是否可选
			if(!isTouching)
				setCurrentItem(Math.min(maxvalue, pos), false);
			else
