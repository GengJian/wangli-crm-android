...
import com.control.widget.xclcharts.event.click.BarPosition;
import com.control.widget.xclcharts.renderer.AxesChart;
import com.control.widget.xclcharts.renderer.XEnum;
import com.control.widget.xclcharts.renderer.bar.Bar;
import com.control.widget.xclcharts.renderer.bar.FlatBar;
import com.control.widget.xclcharts.renderer.info.AnchorDataPoint;
import com.control.widget.xclcharts.renderer.info.PlotAxisTick;
import com.control.widget.xclcharts.renderer.line.PlotCustomLine;
	...
	protected boolean renderVerticalBar(Canvas canvas) {
		...
		for (int i = 0; i < size; i++) {
			...
			for (int j = 0; j < countChild; j++) {
				...
				labelTopY = topY = getVPValPosition(newBv);
				
				if(dataAxis.getAxisStdStatus())
				{	
					if(newBv < dataAxis.getAxisStd()) //反向
					{						 											
				...
				// 在柱形的顶端显示上柱形当前值
				if(!mEqualAxisMin && Double.compare(dataAxis.getAxisMin(), newBv)  == 0)
				{
					if(Double.compare(dataAxis.getAxisMin(), newBv)  == 0){
