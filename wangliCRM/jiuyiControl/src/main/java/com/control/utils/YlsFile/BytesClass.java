...
public class BytesClass {
    ...
    static public String BytesToString(byte[] pData, int nBegin, int nEnd,String Encoding) {
    	try
    	{
    		if (pData != null && (nBegin + nEnd) <= pData.length) {
    ...
    static public int BytesToInt(byte[] pData, int nBegin) throws Exception {
        if (pData != null && (nBegin + 4) <= pData.length) {
            int tmp = (((pData[nBegin + 3] & 0x000000ff) << 24) + ((pData[nBegin + 2] & 0x000000ff) << 16) + ((pData[nBegin + 1] & 0x000000ff) << 8) + (pData[nBegin] & 0x000000ff));
    ...
    public static double BytesToDouble(byte[] pData, int nBegin){    	  
    	  if (pData != null && (nBegin + 8) <= pData.length) {
              long tmp = (
            		  (((long)(pData[nBegin + 7] & 0x000000ff)) << 56) + 
            		  ...
            		  ((pData[nBegin + 1] & 0x000000ff) << 8) + 
            		  (pData[nBegin] & 0x000000ff));
...
//    static public int ReadDataHead(DataHead DataHead, short nType, byte[] pData, int nBegin) throws Exception {
//        if (pData == null && DataHead == null) {
//            return 0;
//        }
//        DataHead.mType = nType;
//
//        if (DataHead.mType == -1) {
//            return nBegin;
//        }
//        DataHead.mIndex = pData[nBegin];
//        nBegin += 1;
//        if (DataHead.mIndex == -1) {
//            return nBegin;
//        }
...
//            return nBegin;
//        }
//        DataHead.mPrivateKey.m_cCodeType = BytesToShort(pData, nBegin);
//        nBegin += 2;
//        if (DataHead.mPrivateKey.m_cCodeType == -1) {
//            return nBegin;
//        }
//        DataHead.mPrivateKey.m_cCode = BytesToString(pData, nBegin, 6);
//        nBegin += 6;
//
//        return nBegin;
//    }
