...
import com.http.interceptor.GzipRequestInterceptor;
import com.http.interceptor.OfflineCacheInterceptor;
import com.http.interceptor.OnlineCacheInterceptor;
...
public class HttpGlobalConfig {
    ...
    public HttpGlobalConfig proxy(Proxy proxy) {
        JiuyiHttp.getOkHttpBuilder().proxy(checkNotNull(proxy, "proxy == null"));
        return this;
    }
    ...
    public HttpGlobalConfig connectTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            JiuyiHttp.getOkHttpBuilder().connectTimeout(timeout, unit);
        } else {
    ...
    public HttpGlobalConfig writeTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            JiuyiHttp.getOkHttpBuilder().writeTimeout(timeout, unit);
        } else {
    ...
    public HttpGlobalConfig readTimeout(int timeout, TimeUnit unit) {
        if (timeout > -1) {
            JiuyiHttp.getOkHttpBuilder().readTimeout(timeout, unit);
        } else {
    ...
    public HttpGlobalConfig interceptor(Interceptor interceptor) {
        JiuyiHttp.getOkHttpBuilder().addInterceptor(checkNotNull(interceptor, "interceptor == null"));
        return this;
    }
    ...
    public HttpGlobalConfig networkInterceptor(Interceptor interceptor) {
        JiuyiHttp.getOkHttpBuilder().addNetworkInterceptor(checkNotNull(interceptor, "interceptor == null"));
        return this;
    }
    ...
    public HttpGlobalConfig cacheOffline(Cache httpCache) {
        networkInterceptor(new OfflineCacheInterceptor(JiuyiHttp.getContext()));
        interceptor(new OfflineCacheInterceptor(JiuyiHttp.getContext()));
        this.httpCache = httpCache;
        return this;
    }
    ...
    public HttpGlobalConfig cacheOffline(Cache httpCache, final int cacheControlValue) {
        networkInterceptor(new OfflineCacheInterceptor(JiuyiHttp.getContext(), cacheControlValue));
        interceptor(new OfflineCacheInterceptor(JiuyiHttp.getContext(), cacheControlValue));
        this.httpCache = httpCache;
        return this;
    }
    ...
    public int getRetryDelayMillis() {
        if (retryDelayMillis < 0) {
            retryDelayMillis = jiuyiConfig.DEFAULT_RETRY_DELAY_MILLIS;
        }
    ...
    public int getRetryCount() {
        if (retryCount <0) {
            retryCount = jiuyiConfig.DEFAULT_RETRY_COUNT;
        }
        return retryCount;
    }
