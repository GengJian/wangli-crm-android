...
import com.http.api.ApiService;
import com.http.callback.ACallback;
import com.http.func.ApiFunc;
import com.http.func.ApiRetryFunc;
import com.http.mode.ApiHost;
import com.http.mode.CacheMode;
...
public abstract class BaseHttpRequest<R extends BaseHttpRequest> extends BaseRequest<R> {
    ...
    protected void generateLocalConfig() {
        ...
        if (isLocalCache) {
            if (cacheKey != null) {
                JiuyiHttp.getApiCacheBuilder().cacheKey(cacheKey);
            } else {
                JiuyiHttp.getApiCacheBuilder().cacheKey(ApiHost.getHost());
            }
            if (cacheTime > 0) {
                JiuyiHttp.getApiCacheBuilder().cacheTime(cacheTime);
            } else {
                JiuyiHttp.getApiCacheBuilder().cacheTime(jiuyiConfig.CACHE_NEVER_EXPIRE);
            }
        }
        if (baseUrl != null && isLocalCache && cacheKey == null) {
            JiuyiHttp.getApiCacheBuilder().cacheKey(baseUrl);
        }
