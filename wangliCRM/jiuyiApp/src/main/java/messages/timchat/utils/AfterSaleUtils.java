...
public class AfterSaleUtils {
    ...
    private static AfterSaleUtils single = null;

    private List<String> searchHistory = new ArrayList<String>();
    ;


    private final String KEY = "afterSaleLooked_history";

    // 静态工厂方法
    public static AfterSaleUtils getInstance() {
        if (single == null) {
            single = new AfterSaleUtils();
        }
        return single;
    }
