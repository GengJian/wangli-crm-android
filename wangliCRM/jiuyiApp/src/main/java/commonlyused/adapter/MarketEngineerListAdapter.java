...
public class MarketEngineerListAdapter extends BaseQuickAdapter<MarketEngineeringBean.ContentBean,BaseViewHolder>  {
    ...
    public MarketEngineerListAdapter(int layoutResId, List<MarketEngineeringBean.ContentBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(final BaseViewHolder holder,final MarketEngineeringBean.ContentBean item) {
    ...
    private MarketEngineerListAdapter.onCLickItemListener listener;

    public void  setOnCLickItemListener(MarketEngineerListAdapter.onCLickItemListener listener)
    {
    ...
    {
        void click(int position, MarketEngineerListAdapter receiveAddress, View view);
    }
    //下面两个方法提供给页面刷新和加载时调用
    public void add(List<MarketEngineeringBean.ContentBean> addList) {
        //增加数据
        int position = getData().size();
        getData().addAll(position, addList);
        notifyItemInserted(position);
    }
    public void refresh(List<MarketEngineeringBean.ContentBean> newList) {
        //刷新数据
