...
public class MarketWorkPlanListAdapter extends BaseQuickAdapter<MarketWorkPlanBean.ContentBean,BaseViewHolder>  {
    ...
    public MarketWorkPlanListAdapter(int layoutResId, List<MarketWorkPlanBean.ContentBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(final BaseViewHolder holder,final MarketWorkPlanBean.ContentBean item) {
...
//        tv_actualShipmentlabel.setText("实际发货");
    ...
    private MarketWorkPlanListAdapter.onCLickItemListener listener;

    public void  setOnCLickItemListener(MarketWorkPlanListAdapter.onCLickItemListener listener)
    {
    ...
    {
        void click(int position , MarketWorkPlanListAdapter receiveAddress , View view);
    }
    //下面两个方法提供给页面刷新和加载时调用
    public void add(List<MarketWorkPlanBean.ContentBean> addList) {
        //增加数据
        int position = getData().size();
        getData().addAll(position, addList);
        notifyItemInserted(position);
    }
    public void refresh(List<MarketWorkPlanBean.ContentBean> newList) {
        //刷新数据
