...
public class MarketNengChengListAdapter extends BaseQuickAdapter<MarketNengChengBean.ContentBean,BaseViewHolder>  {
    ...
    public MarketNengChengListAdapter(int layoutResId, List<MarketNengChengBean.ContentBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(final BaseViewHolder holder,final MarketNengChengBean.ContentBean item) {
    ...
    private MarketNengChengListAdapter.onCLickItemListener listener;

    public void  setOnCLickItemListener(MarketNengChengListAdapter.onCLickItemListener listener)
    {
    ...
    {
        void click(int position, MarketNengChengListAdapter receiveAddress, View view);
    }
    //下面两个方法提供给页面刷新和加载时调用
    public void add(List<MarketNengChengBean.ContentBean> addList) {
        //增加数据
        int position = getData().size();
        getData().addAll(position, addList);
        notifyItemInserted(position);
    }
    public void refresh(List<MarketNengChengBean.ContentBean> newList) {
        //刷新数据
