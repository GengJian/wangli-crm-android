 ...
 *****************************************************************
 */
public class JiuyiNewMarketHuaJueDtailMutiActivity extends JiuyiActivityBase {
    private TextView mtvcomplete;
    private JiuyiButton mbtnsave;
    private String billType="";
    private long Customerid=-1;
    private String bill_type="";
    private JiuyiItemGroup jigClient,jig_attribute;
    ...
    private JiuyiBigTextGroup jigContent,jig_engineerProcessingMatters;
    ...
    private MarketHuaJueBean.HuaJueItemsBean retailChannelItemsBean;
    private String filePath;
    private  File[] files;
    private List<AttachmentBean> attachmentsBeanslist,attachmentsEditBeanslist;
    private Boolean needUpload=false;
    private LinearLayout ll_content,ll_retailcontent;
    private String sAttributeKey="";
    ...
    public void onInit() {
        ...
        mBodyLayout = (JiuyiRelativeLayout) LayoutInflater.from(this).inflate(Res.getLayoutID(this, "jiuyi_activity_newnengchengmulti_layout"), null);
        ...
        if(operType.equals(ViewOperatorType.ADD)){
            retailChannelItemsBean =new MarketHuaJueBean.HuaJueItemsBean();
        }else{
        ...
        jig_attribute = (JiuyiItemGroup) mBodyLayout.findViewById(R.id.jig_attribute);
        jig_attribute.setItemOnClickListener(new JiuyiItemGroup.ItemOnClickListener() {
            @Override
            public void onItemClick(View v) {
                AlertDialog.Builder buidler = new AlertDialog.Builder(JiuyiNewMarketHuaJueDtailMutiActivity.this);
                buidler.setTitle("客户属性");
                final String[] data;
                final List<DictBean> dictBeansList= DictUtils.getDictList("member_attribute");
                if(dictBeansList!=null &&dictBeansList.size()>0) {
                    ...
                    buidler.setSingleChoiceItems(data, -1, new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            String attributeKey="";
                            jig_attribute.setText(data[which].trim());
                            ...
                            retailChannelItemsBean.setMemberAttribute(contentBean);
                            attributeKey=dictBeansList.get(which).getKey();
                            sAttributeKey=attributeKey;
                            if(attributeKey.equals("project")){
                                ll_retailcontent.setVisibility(View.GONE);
                                jig_engineerProcessingMatters.setVisibility(View.VISIBLE);
                            }else if(attributeKey.equals("retail")){
                                ll_retailcontent.setVisibility(View.VISIBLE);
                                jig_engineerProcessingMatters.setVisibility(View.GONE);
                            }else if(attributeKey.equals("project_retail")){
                                ll_retailcontent.setVisibility(View.VISIBLE);
                                jig_engineerProcessingMatters.setVisibility(View.VISIBLE);
                            }
                            dialog.dismiss();
        ...
        ll_retailcontent= (LinearLayout) mBodyLayout.findViewById(R.id.ll_retailcontent);
        jigContent = (JiuyiBigTextGroup) mBodyLayout.findViewById(R.id.jig_content);
        jig_engineerProcessingMatters = (JiuyiBigTextGroup) mBodyLayout.findViewById(R.id.jig_engineerProcessingMatters);
        ...
        if(retailChannelItemsBean !=null) {
            ...
            if(!Func.IsStringEmpty(retailChannelItemsBean.getEngineerProcessingMatters())){
                jig_engineerProcessingMatters.setText(retailChannelItemsBean.getEngineerProcessingMatters());
            }
            ...
            retailChannelItemsBean.setEngineerProcessingMatters(jig_engineerProcessingMatters.getText());
            ...
            if(retailChannelItemsBean.getMemberAttribute()!=null &&retailChannelItemsBean.getMemberAttribute().getValue()!=null){
                jig_attribute.setText(retailChannelItemsBean.getMemberAttribute().getValue());
                sAttributeKey=retailChannelItemsBean.getMemberAttribute().getKey();
                if(retailChannelItemsBean.getMemberAttribute().getKey().equals("project")){
                    ll_retailcontent.setVisibility(View.GONE);
                    jig_engineerProcessingMatters.setVisibility(View.VISIBLE);
                }else if(retailChannelItemsBean.getMemberAttribute().getKey().equals("retail")){
                    ll_retailcontent.setVisibility(View.VISIBLE);
                    jig_engineerProcessingMatters.setVisibility(View.GONE);
                }else if(retailChannelItemsBean.getMemberAttribute().getKey().equals("project_retail")){
                    ll_retailcontent.setVisibility(View.VISIBLE);
                    jig_engineerProcessingMatters.setVisibility(View.VISIBLE);
                }
            }
    ...
    public void setBackActivityBundle() {
        ...
        JiuyiNewMarketHuaJueDtailMutiActivity.this.setResult(1, intent);
    }
    @Override
...
//                break;
    ...
    public boolean check(){
        ...
        if(Func.IsStringEmpty(sAttributeKey)){
            startDialog(DialogID.DialogDoNothing, "", "请选择客户属性！", JiuyiDialogBase.Dialog_Type_Yes, null);
            return false;
        }
...
//            return false;
//        }
//        if(!jigIscompare.bdefautl && Func.IsStringEmpty(jigContent.getText()) && !sAttributeKey.equals("project")){
//            startDialog(DialogID.DialogDoNothing, "", "请输入内容！", JiuyiDialogBase.Dialog_Type_Yes, null);
//            return false;
//        }
        if(sAttributeKey.equals("project")&&billOperType.equals(ViewOperatorType.SPECIAL) && Func.IsStringEmpty(jig_engineerProcessingMatters.getText())){
            startDialog(DialogID.DialogDoNothing, "", "请输入工程处理事项！", JiuyiDialogBase.Dialog_Type_Yes, null);
            return false;
        }
        return true;
