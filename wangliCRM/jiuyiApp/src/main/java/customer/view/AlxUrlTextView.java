...
public class AlxUrlTextView extends TextView{
        ...
        for(int i=0;i<urlCount;i++){
            ...
            spannableString.setSpan(new ClickableSpan() {
                ...
                public void onClick(View widget) {
                    ...
                    mBundle.putString(JiuyiBundleKey.PARAM_HTTPServer, href);
                    String title="";
                    title=blueText;
                    if(title.length()>14){
                        title=blueText.substring(0,14)+"...";
                    }
    ...
    public static Object[][]  getUrlFromJDP(String source){
        ...
        Pattern pattern2 = Pattern.compile("<a href=\".*?\">(.*?)</a>");//首先将a标签分离出来
//        Pattern pattern = Pattern.compile("<a href=('|\"){1}.*?('|\"){1}>(.*?)</a>");//首先将a标签分离出来
...
//            Pattern url_pattern = Pattern.compile("<a href=('|\"){1}(.*?)('|\\\"){1}>");//将href分离出来

            Matcher url_matcher = url_pattern.matcher(raw);
            try {
                if(url_matcher.find()){
                    String url = url_matcher.group(1).replace("target='_blank","");
...
//            Pattern url_pattern = Pattern.compile("<a href=('|\"){1}(.*?)('|\\\"){1}>");//将href分离出来
            ...
            Log.i("Alex","蓝色文字是"+host);//括号里面的
            hosts.add(i,host);
            starts.add(i,matcher2.start());
            ends.add(i,matcher2.end());
            Log.i("Alex","字符串起始下标是"+matcher2.start()+"结尾下标是"+matcher2.end());//匹配出的字符串在源字符串的位置
            i++;
        }
        if(hosts.size()==0){
            Log.i("Alex","没有发现url");
