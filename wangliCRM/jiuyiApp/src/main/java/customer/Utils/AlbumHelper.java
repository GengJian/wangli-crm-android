...
//		bucket.imageList = new ArrayList<ImageBean>();
	...
	public List<ImageBucket> getImagesBucketList(boolean refresh) {
		...
		List<Map.Entry<String, ImageBucket>> infoIds = new ArrayList<Map.Entry<String, ImageBucket>>(   bucketList.entrySet());

		if(infoIds!=null && infoIds.size()>0){
			for(int k=0;k<infoIds.size();k++){
				Entry<String, ImageBucket> entry = infoIds.get(k);
				ImageBucket bucket = entry.getValue();
				if(bucket.bucketName.toLowerCase().equals("camera")){
	...
	public List<ImageBucket> getVideosBucketList(boolean refresh) {
