...
/**
 * ****************************************************************
 * 文件名称:BGAProgressBar.java
 * 作    者:Created by zhengss
 * 创建时间:2018/9/4 11:32
 * 文件描述:
 * 注意事项:
 * 版权声明:Copyright (C) 2018-2025 上海九翊软件科技有限公司
 * 修改历史:2018/9/4 1.00 初始版本
 * ****************************************************************
 */
...
//        mText = String.format("%d", (int) (getProgress() * 1.0f / getMax() * 100)) + "%";
        mText = String.format("%d", (int) (this.progress * 1.0f / getMax() * 100)) + "%";
        int newTextsize=0;
        if(mText.length()>7){
            newTextsize= mTextSize-8;
            mPaint.setTextSize(newTextsize);
        }else if(mText.length()>5){
            newTextsize= mTextSize-4;
            mPaint.setTextSize(newTextsize);
        }else{
